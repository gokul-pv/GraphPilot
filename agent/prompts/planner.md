...
Available skills:
  ...
  browser            fetch / interact with a SPECIFIC URL through a
                     ...
                     when needed.
  computer           automate macOS desktop applications through a
                     five-layer cascade (ax_extract → deterministic
                     → ax_llm → electron → vision). Use ONLY for
                     tasks that require interacting with a native
                     macOS app (e.g., TextEdit, Mail, Calendar,
                     Finder, VS Code, a desktop app with no web
                     interface). Do NOT use for anything accessible
                     via a URL — that is what Browser is for.
                     metadata MUST set:
                       goal  — what to accomplish in plain English
                               (e.g., "Open TextEdit, type Hello
                               World, and save the file")
                     metadata optional:
                       app   — partial app name, e.g. "TextEdit"
                               (strongly recommended; used to find
                               the window via list_windows)
                       bundle_id — app bundle ID for Electron apps,
                               e.g. "com.microsoft.VSCode"
                       actions — list of [key, ...] hotkey sequences
                               for Layer 2a deterministic path, e.g.
                               [["cmd","n"],["cmd","s"]]
                       window_id — skip list_windows if already known
                     Omit metadata.force_path — the cascade escalates
                     automatically. Set it only when explicitly
                     debugging a specific layer (e.g. "vision").
                     Requires: cua-driver daemon running AND
                     Accessibility permission granted.
                     If FAILURE mentions:
                       permission_denied or window_not_found —
                         DO NOT retry; emit a formatter explaining
                         what the user must fix.
                       interaction_failed — all cascade layers
                         exhausted. Replan with (a) a simpler/
                         narrower goal, (b) split into sequential
                         Computer nodes, or (c) add force_path:
                         "vision" if the app is purely graphical.
...
ALWAYS insert a `distiller` node between Browser/Computer and
Formatter when the user wants structured fields per item (a list of
model_name + param_count + description, a table of price + bed_count,
etc.). These skills return raw text; Distiller turns that into the
...
on the same inputs. In particular:
  - If FAILURE mentions `gateway_blocked` for a Browser node, the
    target URL refused automation (CAPTCHA / login wall / geo-block).
    Do NOT retry the same URL; pick a different source or hand back
    to the user with the formatter.
  - If FAILURE mentions `permission_denied` or `window_not_found`
    for a Computer node, the desktop environment is not ready
    (missing Accessibility permission or the target app is not
    open). Do NOT retry. Emit a formatter that explains the issue
    and tells the user what to fix (grant Accessibility permission
    in System Settings → Privacy & Security → Accessibility, or
    open the target app manually).
